...
def login_auth():
    ...
    # Verify the reCAPTCHA response using the verify_recaptcha function
...
def delete_bug(id_data):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("UPDATE bug SET is_deleted = 1, deleted_at = %s WHERE bug_id = %s", (current_time, id_data))

        connection.commit()
        message = f"Bug with ID {id_data} has been marked as deleted"
...
def search_bug():
    ...
    if request.method == 'POST':
        ...
        sql = "SELECT * FROM bug where is_deleted = 0"
        conditions = []
...
# @app.route('/recover_bug/<string:bug_id>', methods=['GET'])
...
#     connection = pymysql.connect(**db_config)
#     with connection.cursor() as cursor:
#         # Update the bug record to mark it as not deleted
#         cursor.execute("UPDATE bug SET is_deleted = 0, deleted_at = NULL WHERE bug_id = %s", (bug_id,))
#         connection.commit()
#         message = f"Bug ID {bug_id} has been recovered."
#         flash(message=message)
#         return redirect(url_for('update_bug'))


@app.route('/recover_bug', methods=['GET', 'POST'])
def recover_bug():
    if "loggedin" not in session:
        message = "You need to log in first"
        flash(message=message)
    ...
    recovery_message = None  # Initialize the recovery message

    # Check if 'bug_id' is present in the query parameters and the request method is POST
    if request.method == 'POST':
        ...
        if bug_id:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute("UPDATE bug SET is_deleted = 0, deleted_at = NULL WHERE bug_id = %s", (bug_id,))
                connection.commit()
                recovery_message = f"Bug ID {bug_id} has been recovered."

    # Continue with the rest of your route handling
    connection = pymysql.connect(**db_config)
    with connection.cursor() as cursor:
        # Retrieve all soft-deleted bugs
        cursor.execute("SELECT * FROM bug WHERE is_deleted = 1")
        soft_deleted_bugs = cursor.fetchall()

    return render_template('recover_bug.html', bugs=soft_deleted_bugs, recovery_message=recovery_message)
